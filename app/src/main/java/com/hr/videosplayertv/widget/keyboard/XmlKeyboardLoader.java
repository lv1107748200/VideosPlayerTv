...
public class XmlKeyboardLoader {
	...
	public SoftKeyboard loadKeyboard(int resourceId) {
		NLog.e(KEY,"loadKeyboard loading ... ...");
		...
		try {
				...
				if (mXmlEventType == XmlResourceParser.START_TAG) {
					...
					if (XMLTAG_KEYBOARD.compareTo(attr) == 0) { // keyboard
						...
						if (softKeyboard == null) {
							NLog.e(KEY,"XMLTAG_ROW softKeyboard null");
							return null;
						}
						...
						if (null == splitter || null == labels) {
							NLog.e(KEY,"XMLTAG_KEYS splitter or labels null");
							return null;
						}
						...
						if (null == softKeyboard) {
							NLog.e(KEY,"XMLTAG_KEY softKeyboard null");
							return null;
						}
			...
			NLog.e(KEY,"loadKeyboard load over");
			return softKeyboard;
		} catch (XmlPullParserException e) {
			e.printStackTrace();
			NLog.e(KEY,"loadKeyboard XmlPullParserException " + e.getMessage());
		} catch (IOException e) {
			e.printStackTrace();
			NLog.e(KEY,"loadKeyboard IOException " + e.getMessage());
		} finally {
			if (xrp != null)
				xrp.close();
		}
