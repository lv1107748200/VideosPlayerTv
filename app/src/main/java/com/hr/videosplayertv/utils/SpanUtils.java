...
public class SpanUtils {
     ...
     * @return
     */
    public SpanUtils setTextClick(CallBack callBack){
        isClick = true;
    ...
    public SpanUtils appendLine(@NonNull final List<CharSequence> text) {
        for(int i =0 ;i < text.size();i++ ){
            apply(mTypeCharSequence);
            mText = text.get(i) + LINE_SEPARATOR;

                 setFontSize(DisplayUtils.sp2px(17));
        }
    ...
    private void updateCharCharSequence() {
       ...
       // System.out.println("--->"+end);
        ...
        if(isClick){
            mBuilder.setSpan(new TextClick(callBack,mText.toString()), start, end, flag);
        }
    ...
    class CustomIconMarginSpan implements LeadingMarginSpan, android.text.style.LineHeightSpan {
        ...
        private Bitmap uri2Bitmap(final Uri uri) {
            try {
                return MediaStore.Images.Media.getBitmap(BaseApplation.getBaseApp().getContentResolver(), uri);
            } catch (IOException e) {
                e.printStackTrace();
        ...
        private Bitmap resource2Bitmap(final int resourceId) {
            Drawable drawable = ContextCompat.getDrawable(BaseApplation.getBaseApp(), resourceId);
    ...
    public class MyImageSpan extends ImageSpan {
        public MyImageSpan(Context arg0, Drawable drawable) {
            super(drawable);
        }
    ...
    class CustomImageSpan extends CustomDynamicDrawableSpan {
        ...
        private CustomImageSpan(final Bitmap b, final int verticalAlignment) {
            ...
            mDrawable = new BitmapDrawable(BaseApplation.getBaseApp().getResources(), b);
        ...
        private CustomImageSpan(final Drawable d, final int verticalAlignment, int right, int bottom) {
            super(verticalAlignment);
            mDrawable = d;
        ...
        public Drawable getDrawable() {
            ...
            if (mDrawable != null) {
                ...
                Bitmap bitmap;
                try {
                    InputStream is = BaseApplation.getBaseApp().getContentResolver().openInputStream(mContentUri);
                    bitmap = BitmapFactory.decodeStream(is);
                    drawable = new BitmapDrawable(BaseApplation.getBaseApp().getResources(), bitmap);
                    drawable.setBounds(0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight());
                    if (is != null) {
                        is.close();
                    }
                ...
                try {
                    drawable = ContextCompat.getDrawable(BaseApplation.getBaseApp(), mResourceId);
                    drawable.setBounds(0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight());
                } catch (Exception e) {
    ...
    class TextClick extends ClickableSpan {
        ...
        public TextClick(CallBack callback) {
            this.callback = callback;
        }

        public TextClick(CallBack callback, String string) {
            this.callback = callback;
            this.string = string;
        }
